...
public class OptimoaBT {
	...
	public void antartida(int i, double SPD, int[] SPL, int SPxP, int SPxT) {
		if(SPxP==0 || i>N) {
			if(SPD>SOD) {
				SOD=SPD;
				SOL=SPL.clone();
			...
			if((SPxT-bidaia.bidPisua(i)>=0) && diruLista[i]+SPD>SOD ) {
				
				SPL[i]=1;
				antartida(i+1,SPD+ bidaia.bidDirua(i),SPL,SPxP-1, SPxT-bidaia.bidPisua(i));
				SPL[i]=0;
			}
